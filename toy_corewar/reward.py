...
class Specific_register_values(Reward_function):
    def reward(self, env):
        initial_dist = self.circular_distance(env.register_states[-2], self.targets)
        final_dist = self.circular_distance(env.register_states[-1], self.targets)
        improvement = initial_dist - final_dist
        return improvement.sum()

    def performance(self, env):
        dist = self.circular_distance(env.register_states[-1], self.targets)
        return -dist.sum()

    def circular_distance(self, before, after):
        diff = before - after
        return np.minimum(abs(diff), diff % 256)
