        ...
        while True:
            ...
            if episodeStep == PREDICTION_LEN:
                ...
                for i in range(NUM_COMPETITORS):
                    ...
                    r, ones, twos, p1, p2 = self.game.getGameEnded(self.curProgram, self.curOpponent[i])
                    reward += r
                    if self.savePrograms:
                        turn_program_into_file(p1, self.file_path + '/' + str(self.cpuId) + '-' + str(self.episode) +
                                               '-' + self.print_winner(r) + "-tiles-" + str(ones) + "-" + str(i) + "-p1.rle")
                        turn_program_into_file(p2, self.file_path + '/' + str(self.cpuId) + '-' + str(self.episode) +
                                               '-' + self.print_winner(-r) + "-tiles-" + str(twos) + "-" + str(i) + "-p2.rle")
                self.wins += reward
                reward /= NUM_COMPETITORS
                return[(x[0], x[1], reward) for x in trainExamples]
