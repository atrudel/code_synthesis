...
class MCTS():
    ...
    def getActionProb(self, program, iterations, gpuId, temp=1):
        
        for i in range(NUM_MCTS_SIMS * iterations):
            program_sim = deepcopy(program)
            self.search(program_sim, gpuId)
    ...
    def search(self, program, gpuId):
        
        s = self.game.stringRepresentation(program)
        
        if program[-1] != -1:
            _, v = self.nnet.predict(self.game.neuralNetworkInput(program), gpuId)
            return v
                
        if s not in self.Ps:
            self.Ps[s], v = self.nnet.predict(self.game.neuralNetworkInput(program), gpuId)
            self.Ns[s] = 0
            return v
        ...
        v = self.search(next_program, gpuId)
